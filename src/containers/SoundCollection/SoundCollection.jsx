    ...
    /*const handleLike = async (sample) => {
        try {
            const endpoint = sample.cur_liked
                ...
                : `https://samplevault.ru/api/v1/sounds/like/${sample.id}`;
            ...
            setSamples(samples.map(item => {
                if (item.id === sample.id) {
                    return { ...item, cur_liked: !item.cur_liked };
                }
                return item;
            }));

            console.log(`Track successfully ${sample.cur_liked ? 'removed from' : 'added to'} favorites`);
        } catch (error) {
    ...
    return (
        <div className='collection-container'>
            ...
            {filteredSamples.length ? filteredSamples.map((sample, index) => (
                <div className="containersample" onClick={() => handleSongClick(filteredSamples, index)} key={sample.id}>
